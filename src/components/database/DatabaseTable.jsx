        ...
        <thead>
          <tr className="bg-muted/40 border-b border-border">
            ...
            {visibleProps.map(prop => (
              <th
                key={prop.id}
                className="border-r border-border relative group"
                style={{ width: colWidth(prop), minWidth: colWidth(prop) }}
              >
                <button
                  className="flex items-center gap-1.5 w-full px-3 py-2 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors"
                  onClick={() => onEditProperty(prop)}
                >
                  <span className="truncate">{prop.name}</span>
                  <span className="text-[10px] text-muted-foreground/50 hidden group-hover:inline">{PROPERTY_TYPES[prop.type]?.label}</span>
                </button>
                {/* Resize handle */}
                <div
                  className="absolute right-0 top-0 w-1 h-full cursor-col-resize hover:bg-primary/50 opacity-0 group-hover:opacity-100 transition-opacity"
